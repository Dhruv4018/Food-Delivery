...
export const verifyPayment = async (req, res) => {
    try {
        ...
        // socket event online ke liye hai
        const io = req.app.get('io')
        if (io) {
...
export const acceptOrder = async (req, res) => {
    try {
        ...
        if (assignment.status !== "brodcasted") {
            return res.status(400).json({ message: "Assignment is expiry " })
        }
...
export const getOrderById = async (req, res) => {
    try {
        ...
        if (!order) {
            return res.status(400).json({ message: "getorder not found" })
        }

        return res.status(200).json(order)
    } catch (error) {
        return res.status(500).json({ message: ` order getby id error ${error}` })
    }
}
...
export const sendDeliveryOTP = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: ` otp order  id error ${error}` })
    }
}
...
export const verifyDeliveryOtp = async (req, res) => {
    try {
        ...
        return res
            .status(500)
            .json({ message: `verify otp order id error ${error}` });
    }
};
...
export const getTodayDeliveries = async (req, res) => {
    try {
        ...
        return res.status(200).json(formattedStats)
         ...
         return res.status(500).json({ message: `today delivered error ${error}` })
    }
}
